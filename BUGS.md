...
- **B-013** — Once a PERFORMANCE tempo is set for a spot in one strategy, preload it for that spot in all other strategies. (per-passage performance-tempo memory shared across strategies.)
  - **2026-07-14 BUILT (code, not pushed) — ⚠️ Supabase migration must run BEFORE/AT the next web push:** `alter table pieces add column if not exists performance_tempo integer;` (also in db/schema.sql; needs Ralph's OK to run on prod). SQLite migration ships in the build and runs itself. Web is deploy-order-safe even if the migration is missed: the write-back is best-effort (catch + warn), and reads use `select *` so a missing column just means no prefill.
