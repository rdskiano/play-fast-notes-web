...
- **B-018** ✅ CALIBRATED + BUILT 2026-07-14 (code, not pushed) — Rhythm-exercise tempo had no relationship to the passage's tempo (both modes hard-coded 80 BPM), and the dial's denominator-based meaning made the felt speed lurch between meters. Ralph's ear-calibration (same day, conversational) settled the rule: **quarter-meters at the goal tempo; the eighth note at 1.5 × goal in every other meter** (goal 140 → 2/4 at ♩=140, 3/8·5/8·7/8 at ♪=210). Built: `meterTempoFactor` + seeding from `pieces.performance_tempo` + ratio-rescale on every pattern/grouping/card change (manual nudges carry across meters); engine BPM ceilings 300/400→600 for /16 dials. Full history in **RHYTHM_TEMPO_PLAN.md**. **Verify on web after push:** 140-goal passage → Rhythm Variations grouping 4 → dial reads 210; arrow into the 2/4 patterns → dial becomes 140 and the exercises FEEL like one consistent tempo; nudge to 200 in 3/8 → arrow to 2/4 → ~133 (felt tempo kept). Passages with no goal tempo yet keep the old 80 start.
