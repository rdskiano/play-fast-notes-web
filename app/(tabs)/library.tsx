...
export default function LibraryScreen() {
  ...
  // Which side of the little Add window is showing: null = the menu,
  // 'pick'/'scan' = the in-card add-a-part flow (AddPdfFlow). The flow sets
      ...
      <AddChooserModal
        ...
        onFlowDone={(docId) => {
          ...
          if (docId) {
            ...
            // Batch add stays in the library — pull the new parts in.
            void refresh();
          }
        }}
        onPickPassage={async () => {
          ...
          // Flip the little Add window to the in-card photo flow.
          setAddFlow('photo');
        }}
        onPickDocument={() => {
          if (!entitlement.isPro) {
        ...
        onPickScan={() => {
          ...
          // and scanning already lived behind the Pro-gated Add PDF button.
          if (!entitlement.isPro) {
            setAddOpen(false);
            setPaywallContext('Scanned full parts are a Practice Pro feature.');
            return;
          }
          setAddFlow('scan');
        }}
...
function AddChooserModal({
  ...
  onFlowDone: (docId: string | null) => void;
  onPickPassage: () => void;
  onPickDocument: () => void;
  onPickScan: () => void;
  onPickFolder: () => void;
  ...
  const scheme = useColorScheme() ?? 'light';
  const C = Colors[scheme];
  // Option rows say where each path goes, so "Add PDF" no longer lands on a
  // surprise chooser screen. Scanning is its own entry (native only — the
  // camera scanner doesn't exist on web) so it never ambushes the PDF path.
  const options: {
  ...
  }[] = [
    {
      icon: 'image',
      label: 'Add a photo',
      desc: 'Snap or choose a photo of the page',
      ...
      ? [
          {
            ...
            label: 'Scan paper pages',
            desc: 'Camera scan, cleaned to black and white',
            onPress: onPickScan,
          },
        ]
      : []),
      ...
      <View style={styles.modalBackdrop}>
        <View style={[styles.modalCard, { backgroundColor: C.background }]}>
          {flow === 'photo' ? (
            ...
            // The other side of the little window: pick/scan → name → add,
            // all inside this same card. `key` remounts the flow when the
            // user re-enters it so the picker auto-opens fresh each time.
            <AddPdfFlow
              key={flow}
              start={flow}
              folderId={folderId}
            ...
            <>
              ...
              {options.map((o) => (
                <Pressable key={o.label} onPress={o.onPress} style={styles.addOpt}>
                  <View style={styles.addOptIcon}>
                    <Feather name={o.icon} size={17} color={Palette.accentDeep} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <ThemedText style={styles.addOptLabel}>{o.label}</ThemedText>
                    {o.desc != null && (
                      <ThemedText style={styles.addOptDesc}>{o.desc}</ThemedText>
                    )}
                  </View>
                </Pressable>
              ))}
...
const styles = StyleSheet.create({
  ...
  addOptLabel: { fontSize: Type.size.md, fontWeight: Type.weight.bold },
  addOptDesc: { fontSize: Type.size.xs, color: Palette.textSecondary, marginTop: 1 },
  toastAnchor: {
    position: 'absolute',
    left: 0,
    right: 0,
