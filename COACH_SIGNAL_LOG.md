...
## SESSION 6 SUMMARY (closed — Ralph done for the day)

Longest and richest narrated session yet: D54–D65 + F26–F29 in one day.

**The two concept-level findings:**
1. **Practice MODES (D57/D59/D60):** the coach knows only "learning" mode.
   Observed today: MAINTENANCE (daily routine = attendance receipts via a
   ritualized 1-rep ladder — trail data is ceremony, not practice) and
   POLISHING (played-before repertoire — probe skipped, passage list
   generated by an error-driven play-through that doubles as D49's scope
   declaration). Candidate one-tap bridge: "New to you, or played it
   before?" at piece creation.
2. **The routing PRINCIPLE (D61/D62):** "the tool supplies what the
   material lacks." Three difficulty axes observed: evenness→RV,
   interval security→ICU, integration ("processing")→straight ladder.
   A probe can re-diagnose the axis (D63 coda). Trigger stays a visual
   read the app can't make — tag-at-marking is the standing bridge idea.

**Nearly encodable (queue for next refinement batch, Ralph sign-off
pending):** Micro-timer offer on big ICU sessions (D64 — trigger AND
treatment live-validated, totalSteps known at launch); post-session
"split the scope" echo (D64 coda); surfacing unused capabilities like
ICU backward (trail carries directions[]). All pedagogy-free offers.

**New feature with complete ratified shape:** D58 prompt timer (fifth
timer: user-set interval, random phrase from ONE personal list,
auto-fade banner; teacher-prescribed lineage: the breath-builder story).

**Two-axis rep outcome (D63):** clean-but-panicked rep → Ralph clicked a
FALSE MISS (then bailed, so it never persisted). "Clean — solid or
shaky?" is the candidate fix. Evaluation flow is all-or-nothing on
back-out (nothing persists — good against corruption, lossy on real
info).

**Strategy settled:** PFN = best RECEIVER of PDFs, not a Genius Scan
rival (their auto-fold/edge detection is the paid crown jewel; PFN scan
= Apple's stock scanner, fails on FOLD LINES which no generic detector
handles). Priority: F29 batch upload (OTA-able) → F28 share-sheet target
(native build) → post-scan crop-to-fold using house crop UI.

**Boundaries Ralph drew (do NOT code):** A-vs-B♭ clarinet judgment
(D54); step-skipping expertise (D65 — keep ICU steerable, never score
completion); D42 stands over everything.

**Bugs/friction:** F26 = F12 save-crash again (one redone stint, no
loss); F27 viewer metronome hard-resets to 120 (wants tempo memory);
D56 ladder refuses increment 0 (no maintenance mode; workaround creeps
+1); F30 battery (below). **Owed:** ladder-vs-ICU comparison verdict on
reh. 410 (Ralph will report); D53 fix-shape choice still Ralph's call —
today strengthened fix (c), build the daily-exercises feature.

### F30 — Four-hour practice session drained the iPad battery ~100%

Reported at close of Session 6. ~25%/hour under continuous practice use
(screen always on, metronome audio much of the time, PDF viewing, one
~12-min 141-step ICU session, several tools). Not yet diagnosed — could
be within normal range for continuous screen-on use on an aging iPad, or
could point at app waste (suspects to check in a diagnostic session, per
the warm-screens lesson [[feedback_warm_screens_keep_running]]: metronome
lookahead/tick loops running while STOPPED; audio context alive on idle
screens kept warm by the router; reanimated/animation loops; keep-awake
scope). First step is calibration, not code: how fast does the same iPad
drain doing video/forScore for comparable time? Asked Ralph for baseline
feel + iPad age. No conclusions yet.

### D59 — SECOND sighting of the D52 routing rule: running 16ths → rhythm
### exercises as the OPENING tool (and the probe step gets skipped)

- **What Ralph did (his words):** starting the Classical 1 program with the
  Cuban Overture — "some 16th-note passages in here that are sort of
  notoriously tricky, and they're running 16th notes. I just went ahead and
  created a rhythm exercise for each of them, and that's how I'm practicing
  them starting out."
- **Why it matters:** D52 (Session 5, passage "104") observed exactly this
  routing once: running 16ths → rhythm-variation family, chosen off the
  VISUAL character of the passage (D51's invisible signal). This is an
  independent repeat on different repertoire, unprompted. One observation
  is an anecdote; two is a candidate rule — but the trigger (the user SEES
  running 16ths) remains invisible to the app. Possible bridge per D9: a
  cheap character tag at marking time ("running 16ths" chip) could hand
  the coach the signal without notation analysis. NOT ratified.
- **Tension with Session 1 rule 3 (first practice = probe at goal) —
  CONFIRMED, and Ralph named the missing dimension:** "Yes, I went right to
  the rhythm tool because I've played this piece before. Right now I'm just
  polishing it. I'm not learning it for the first time. I guess that's a
  very big difference." **LEARNING vs POLISHING.** The probe-at-goal rule is
  a LEARNING rule; polishing skips discovery because the player already
  holds a complete difficulty map from prior performances. Critically, the
  app cannot infer this: a just-added Cuban Overture has an empty trail
  (prior seasons predate the app), so cold-start TRAIL ≠ cold-start
  MUSICIAN. Every first-practice rule the coach owns silently assumes
  learning mode. Candidate bridge (NOT ratified): one cheap question at
  piece/passage creation — "New to you, or played it before?" — same
  family as D49's scope declaration; would fork the evaluation flow
  (learning → probe; polishing → straight to tools, reputation pre-judged).
  Orchestral life makes this common, not edge-case: rep returns season
  after season.

Context from Ralph at the top of the session: "I feel like I've fallen off the
wagon a little bit" (meaning to be clarified — asked). Session is live; entries
below appended as they happen.

### D54 — Instrument-within-instrument: warm up on the A clarinet

- **What Ralph did (his words):** doing today's warm-up exercises on his A
  clarinet "because it's bigger. When I'm comfortable on the A clarinet, I'm
  always comfortable on the B-flat, but when I'm comfortable on the B-flat,
  I'm not necessarily comfortable on the A. I tend to prioritize being
  comfortable on the A."
- **What the app would have known:** nothing. The Daily Routine passages carry
  no record of WHICH clarinet a stint was played on — the trail for "Scales at
  ♩=X" is identical whether it was the A or the B-flat. The app's instrument
  concept is per-account/per-passage, not per-session.
- **Verdict: NO — signal absent, and whether it SHOULD exist is Ralph's call.**
  His actual rule is a transfer asymmetry (A-comfort ⇒ B♭-comfort, not the
  reverse), which he already runs himself with zero cognitive load. Open
  questions logged, not answered: does a tempo climbed on the B♭ "count" for
  the A? Would he ever want per-session instrument tags, or is this exactly
  the kind of judgment the app should stay out of? Do NOT encode.

### D58 — Ralph's idea: a prompt timer — preloaded coaching cues that
### surface randomly while the metronome runs (no session required)

- **What Ralph said (mid-exercise, his words):** "kind of wishing that maybe
  there was another timer that I could preload with certain prompts from
  myself or from teachers. It would randomly show up on the screen every so
  often. Maybe it could be just a banner across the top that momentarily
  covers the buttons while the metronome is going." His example prompts:
  support the air from the diaphragm · relax the throat · don't grip with
  the bottom lip · always shape the phrase · do you feel tension in your
  back? — "reminders to keep me focused on the… analytical brain parts of
  doing this that I need to be addressing in the practice room."
- **Why this is D9 coming true:** Session 1 predicted quality dimensions
  (tone, body, phrasing) are INVISIBLE to the rep-and-tempo trail and would
  have to ride as CONTENT, not data — "coach whispers / passage tags." This
  is Ralph independently requesting exactly that delivery channel, with the
  content authored by him/his teachers, not by the app. The app's job is
  timing + surfacing, never pedagogy.
- **The practical wrinkle he flagged himself:** he's practicing off the PDF
  with the metronome going and NO session running — so the feature can't
  hang off session state. Rhymes with D57: for real practice, metronome-on
  is the truest "practicing now" signal the app has. Mechanism-adjacent
  machinery already in the app: the four practice timers (Rotate/Micro/
  Cold/Break) live in the tools layer and fire on intervals without needing
  a session; a prompt timer is structurally a fifth. The existing
  per-passage reminders system is content-adjacent (coach reads reminders)
  but its delivery moment is setup-time, not mid-playing.
- **Ratified by Ralph at the stand (2026-09-01) — shape now complete:**
  fires at a PREDETERMINED interval (user-set); picks AT RANDOM one phrase
  from the loaded list; banner AUTO-FADES (hands are on the instrument);
  ONE personal prompt list for now (not per-exercise). Lives as a fifth
  timer alongside Rotate/Micro/Cold/Break. Only remaining open thread:
  teacher-authored/shared prompts (far-off, teaching-studio angle).
- **Pedagogical lineage (his memory, same moment):** teachers told him that
  when working on a more relaxed air stream, "set a timer for every 10
  minutes. When the timer goes off, pick up my breath builder and do a
  couple exercises with it and then continue. It's something that simple."
  So prompts aren't only mental cues — some are micro-ACTIONS (grab a
  device, do a thing, resume). The Break timer is in-app precedent that
  interval interruptions of exactly this kind already work. This grounds
  the feature in real teacher-prescribed practice, not app invention.
- **Verdict: idea captured, NOT committed.** Ralph: "I don't know how it
  would work practically." No build without his ratified shape.

### D60 — Polishing-mode passage discovery: the diagnostic play-through

- **What Ralph did (his words):** "I don't know if this is right or not,
  but I'm basically trying to play through it. Anything that I mess up on,
  I'm marking as a passage." (Cuban Overture, polishing mode per D59.)
- **What this is:** in polishing mode, the passage list is generated by an
  ERROR-DRIVEN PLAY-THROUGH — play the piece, every stumble becomes a
  marked passage. Contrast learning mode, where passages get marked from
  visual read/reputation before playing (D59). The play-through is a
  diagnostic triage pass, and it is entirely invisible to the app: no
  session runs, no tool is open; the only trace is passages appearing
  one by one.
- **Why it matters for D49:** this IS scope declaration, happening
  organically. The play-through produces exactly the denominator the
  piece-level coach needs ("mark all the passages you'll need to
  practice") — in polishing mode no separate declaration step would be
  needed; the triage run generates it. A polishing-mode piece coach could
  even frame it: "play it through, mark what breaks, then we'll work the
  list."
- **Verdict: logged as observed behavior, NOT validated pedagogy.** Ralph's
  own flag: "I don't know if this is right or not." Same status as D49's
  refresh-then-frontier — his real workflow, his real doubt; do not encode
  as a rule without his sign-off. But note the convergence: two separate
  self-doubted workflows (D49, D60) both orbit the same missing feature —
  a piece-level coach that holds the passage LIST, not just each passage.

### D61 — The gut router reads WHAT KIND of hard: intervals → reps first,
### rhythm-mixing second (Cuban Overture, into rehearsal 16)

- **What Ralph did (his words):** stumbled on the measures going into
  rehearsal 16 — "running 8th notes with strange articulations, and my gut
  instinct was to go to interleaved click-up, but it's also a good
  candidate for the rhythmic variation, so I think it's kind of a toss-up.
  I can't say why my gut is telling me to go to interleaved click-up,
  except that there are some weird intervals that maybe would be helpful
  to get some repetition on before I start mixing up the rhythms."
- **Contrast with D52/D59:** running 16ths → rhythm variations FIRST. Here,
  running 8ths + strange articulations + WEIRD INTERVALS → ICU first. The
  distinguishing variable looks like the difficulty AXIS, not the degree:
  evenness/speed problems get attacked by rhythm manipulation; pitch/
  interval-security problems get attacked by repetition. His router reads
  what KIND of hard a passage is — a diagnosis the app cannot make from
  an image or a trail. Tag vocabulary emerging across sightings: "running
  16ths" · "weird intervals" · "strange articulations."
- **The sequencing nuance:** he calls it a toss-up, but his own rationale
  is a SEQUENCE, not a coin flip — repetition to secure the intervals
  FIRST, "before I start mixing up the rhythms." I.e., ICU then RV as a
  pipeline on the same passage. (Asked whether he actually plans to return
  for the RV pass — answer will tell whether this is a real two-stage
  treatment or ICU simply wins.)
- **Also worth keeping:** the gut outran the explanation ("I can't say
  why… except…"). The tacit router is ahead of his own verbal rules —
  the reason D42 stands: the coach encodes only what Ralph can ratify in
  words, which trails what his hands already know.

**RESOLVED same sitting — Ralph articulated the underlying principle after
reading the reflection back:** "The reason that I reached for the ICU tool
was because it [this passage] was not repetitive in any way, and the other
one [the running 16ths] was very repetitive. ICU would have been a waste
of time because it would have basically been the same beat over and over…
I thought it would be more helpful to take that beat and turn it into
something that had a lot of variety."

- **THE PRINCIPLE: the tool supplies what the material LACKS.** Repetitive
  material (running figures, same beat over and over) lacks variety → give
  it rhythm variations. Varied material (weird intervals, changing
  figures) lacks repetition → give it ICU reps. This replaces the
  surface-level "16ths→RV / intervals→ICU" pattern-match with one
  complementarity rule that explains BOTH sightings. It is also the first
  coach-routing rule Ralph has stated in his own words as a principle
  rather than a case. Still needs his explicit sign-off before encoding
  (D42), and the input (repetitive vs varied material) remains a visual
  read the app can't make — tag-at-marking remains the candidate bridge.
- **Multi-tool return confirmed as a value:** "I would probably come back
  and do rhythm variations on this passage, just because I think it's
  great to try different approaches on the same passage." Sequencing =
  secure first, vary second; variety-of-approach is itself part of his
  method (rhymes with D15/D55 anti-rut).
- **Live experiment to watch:** "one pass of ICU in one direction, and
  then I'm going to reverse it, just because I've done very little of it
  in reverse and I want to see what it feels like." REVERSE ICU — unclear
  whether the tool supports running the sequence back-to-front as a mode
  (BACK exists as a step, not a direction). Watch for friction + his
  verdict on whether reverse felt useful.

**Reverse-ICU verdict (Ralph, after doing it):** "I like the option to go
in reverse. It's pretty cool. It genuinely feels different in my brain, so
I think when someone has the time, it's a useful thing. I do think going
forward is probably a better place to start and to spend the most time,
but I do see some value in going in reverse." → Positive but SECONDARY:
forward = the default and the bulk of the time; reverse = a
worth-having variation for when there's time. Mechanism: BUILT IN — ICU
has a native direction option (`ClickUpDirection = 'forward' | 'backward'`
in lib/strategies/clickUp.ts; backward correctly reverses the build
order). No feature gap, zero friction; Claude's tool-model was stale, not
the app. Signal for the coach instead: Ralph had "done very little of it
in reverse" until today — an existing capability going mostly unused is
exactly the kind of thing a variety-suggesting coach (D15) could surface
("you always run this forward — try backward?") without encoding any
pedagogy about WHEN backward is right.

### D62 — Third difficulty axis: "processing" — integration is the problem,
### so the ladder supplies continuity (West Side Story, rehearsal 410)

- **What Ralph did (his words):** "this passage is making me think I should
  do a little bit of tempo ladder. I think the reason is because no small
  section of it is particularly difficult, but weaving the small pieces
  together feels like a processing issue for me. I think I'm just going to
  play it straight and climb the tempo a little bit, and then I'll come
  back and try some ICU with it and see how it feels."
- **Fits the D61 principle (tool supplies what the material lacks):** the
  parts are fine; the LACK is whole-passage continuity under tempo — so
  the tool is playing it STRAIGHT and climbing (tempo ladder), not
  chopping it into units. Third distinct difficulty axis now on record:
  evenness/speed → rhythm variations (D52/D59) · interval security → ICU
  reps (D61) · integration/"processing" → tempo ladder straight-through
  (D62). Tag vocabulary grows: "weaving together" / "processing issue."
- **Two-stage treatment again, now with an explicit comparison intent:**
  ladder first, then "come back and try some ICU with it and see how it
  feels" — he's A/B-testing tools on the same passage (variety-of-approach
  as method, per D61). Comparison verdict owed.
- **Meta-observation:** six hours into the narrated session his
  diagnosis→lack→tool chain has become fluent and articulate — the logging
  practice itself is surfacing the tacit router (he derived the D61
  principle from reading a reflection back). The signal log is functioning
  as the educating instrument, in both directions.

### D63 — Clean but PANICKED: execution and security are different axes,
### and the trail only records one (West Side Story reh. 410, evaluation)

- **What Ralph did (his words):** ran the assessment, set the performance
  tempo, "played it through and didn't make a mistake. I was definitely
  panicked in my mind the whole time, so my instinct is to say that I
  missed it. That's interesting."
- **What the app would have known:** a clean probe at performance tempo —
  by every rule we have, that passage is DONE (Session 1 rule 3: probe at
  goal → clean = done). The trail is binary: clean or miss.
- **What the app could NOT know:** the rep was fragile — white-knuckled,
  not owned. Ralph's musician-instinct wants to score a mentally-panicked
  clean as a MISS, because he knows a performance under adrenaline will
  not get eleven lucky passes. Execution (did the notes happen) and
  SECURITY (was it in control) are separate axes; the clean/miss button
  collapses them, and the collapse is lossy in the dangerous direction —
  it tells the coach to stop working exactly when the player knows better.
- **Candidate bridge (NOT ratified):** after a clean evaluation rep, one
  optional follow-up — "Clean. How did it feel? SOLID / SHAKY" — a
  two-axis outcome. Shaky-clean routes like a near-miss (keep working,
  maybe drop a notch) instead of like done. Cheap, honest, and keeps the
  objective record true instead of forcing Ralph to falsify a miss to
  encode a feeling. Open: does shaky-clean deserve its own treatment rule
  at all — his call.

**Answered — he clicked MISS.** The falsification is now real data: the
trail holds a false miss at ♩=132 on a rep whose notes were clean. The
app's maker chose to corrupt the objective record rather than let a
fragile rep count as done — strongest possible evidence the two-axis
outcome is a real need, not a nicety.

**Follow-on (same moment):** dropped the dial to 100 and "could really
feel the shakiness" — slowing down made the insecurity palpable that
speed had masked. Then a ROUTER OVERRIDE of the D62 plan: "If I were not
practicing with this app, I would probably just move to ICU now. I
wouldn't even try the tempo ladder because I feel like in ICU it's going
to force me to play it slower to start out with than I would have
otherwise." The probe UPDATED the diagnosis: what reh. 410 lacks isn't
continuity after all — it's security → ICU (consistent with the D61
principle; the axes fork treatment, and a probe can re-diagnose the
axis). Also a new tool-property observation: ICU's value here is partly
that it FORCES a slower start than his own judgment would pick —
the tool as a commitment device against his own tempo optimism.

**Live test he initiated:** backing out of the evaluation flow entirely
mid-stream — "We'll see if it logged any tempo information on this
passage." **DB verdict (checked live): NOTHING persisted.** Passage "410"
(created 15:27) has performance_tempo NULL and zero practice_log rows.
The 132 goal, the Miss click, the drop to 100 — all evaporated on
back-out; the evaluation flow is all-or-nothing. Two-sided finding:
(GOOD) the false miss never entered the trail, so D63's corruption
worry is self-limiting when the user bails; (BAD) a real, informative
probe happened (clean-but-panicked @132, shaky @100) and the app retains
none of it — the coach will treat 410 as never-touched. Abandon-the-flow
= the app forgets the session ever happened, which is itself a lossy
default worth a design look (e.g., persist a lightweight "probed, not
locked" breadcrumb). NOT ratified.

**Also confirmed in the same DB pull:** the reverse-ICU experiment is
faithfully recorded — "Into 16" click_up row @15:14, `passes: 2,
directions: ["forward","backward"]`, tempo 180, 77 steps, ~11.5 min. The
trail CAN see direction usage — a variety-suggesting coach (per the D15
note above) has the data it needs for "you never run this backward."

### D64 — Micro-timer self-administered at step 46 of 141; Ralph proposes
### the trigger himself: unit-count threshold (mid-ICU on "410")

- **What Ralph did (his words, narrated mid-session):** "I've got 141
  different steps I need to take in this practice session, and I'm on 46.
  I can feel my brain is tired, so I automatically went up and just
  clicked the micro timer. Let's see if that helps. If it does, that
  could be a suggestion that the coach could offer once it senses that
  you have crossed a threshold of how many units you've marked."
- **Why this one is nearly encodable:** this is the SECOND live
  self-validation of micro-during-long-sittings (first noted in the D53
  review as "safely encodable"), and this time Ralph supplied the trigger
  design himself: session SIZE in steps/units, not wall-clock. Key data
  fact: the app knows totalSteps AT SESSION START (click_up rows carry
  step/totalSteps — verified in today's DB pull, e.g. 77 for "Into 16")
  — so the coach could suggest arming the Micro timer when a big ICU
  session BEGINS, or nudge mid-way. Observed fatigue onset today: ~1/3
  through (46/141).
- **Rule sketch (needs only his sign-off + threshold number):** ICU
  session with totalSteps ≥ N → suggest/offer the Micro timer at start.
  Suggestion-of-an-existing-tool, pedagogy-free (same safe class as the
  backward-direction surfacing note above). **Verdict delivered: "The
  break really helped."** Both halves now validated by live use — the
  trigger (step-count threshold) and the treatment (micro-break). This
  rule is as ratified as anything in this log gets short of Ralph's
  formal design-session sign-off; queue it for the next refinement batch.

### D65 — Skipping ICU steps mid-session: expert override, explicitly
### marked NOT-TO-CODE by Ralph

- **What Ralph did (his words):** "I also found myself skipping phases
  because of the nature of this passage. I could just tell that it was
  going to be a waste of time to play the units as I had marked them, but
  that's just an experience thing. I don't think that's anything we could
  code."
- **Why log a thing we won't build:** the negative space matters. This is
  Ralph drawing the D42 boundary himself — the marked-unit sequence is a
  DEFAULT SCRIPT, and experienced players route around it in real time
  using judgment the trail can't see (he skipped combinations he could
  "just tell" were dead time). The coach should never police completion
  or treat skipped steps as a signal of trouble.
- **The design principle worth keeping:** the tool's job is to stay
  STEERABLE. ICU earns expert trust precisely because NEXT/BACK/skip let
  experience override the script without friction. Any future "did you
  finish the sequence?" logic, completion scoring, or skip-shaming would
  break the tool for its best users. Skipped ≠ struggling; sometimes
  skipped = expertise.

**D64/D65 coda — the second remedy for an oversized session:** "I also
just gave myself a note to do half as many next time. I'm going to do
half a phrase at a time." So the full response to the 141-step session
was two-pronged: mid-session micro-break (D64) + NEXT-session scope
reduction, delivered to his future self through the app's own note
channel. The note loop is doing exactly the D9/D58 division of labor —
his judgment as content, the app as courier. Also completes the fatigue
arc as a coachable pattern (unratified): very large totalSteps →
fatigue → the fix Ralph actually uses is "split the scope next time,"
which a coach could echo as a post-session suggestion on any oversized
ICU run — content it would only ever phrase as an offer, never a rule.

### F28 — Share-sheet PDF import missing, hit live during concert prep

Adding music for a real concert, Ralph tried to export a PDF from another
app straight into Play Fast Notes and PFN "is not coming up as one of the
possibilities" in the iOS share sheet. This is the known wishlist item from
the scan-flow rework ([[project_pfn_scan_flow_rework]] — "forScore
share-sheet") now blocking a real task at the stand. Requires registering
the native app as a PDF share/document target (share extension or document
types) — a NATIVE build change, not OTA-able, which is why it's been
parked. Workaround given live: other app → Save to Files → PFN Add music →
full part → pick PDF. Signal: first observed real-world demand; concert
prep is exactly the moment a player meets this wall.

**Source identified (Ralph, same sitting): Genius Scan.** His scanning
pipeline means THREE copies of every piece: (1) the extensive library
inside Genius Scan, (2) the Save-to-Files copy on the iPad, (3) the copy
inside PFN. The share-sheet target would collapse hop (2).

**Why Genius Scan stays home base (Ralph, verbatim quality report):**
partly habit ("the one that I've been using forever"), but mostly quality —
"the scanning feature is just so much better. The edge detection almost
never fails, whereas the edge detection in my app fails, I would say, 70%
of the time." A 70% edge-detection failure rate in PFN's scan flow is a
first-hand data point from the app's own maker practicing daily.

**Strategic verdict (follows directly, Ralph-sourced):** don't chase Genius
Scan on scanning — make PFN the best RECEIVER of PDFs instead. Priority
order this implies: F29 batch upload (OTA-able, reachable) → F28
share-sheet target (native build) → scan-flow improvement behind both.
The dream gesture: select a whole concert program in Genius Scan → share →
PFN → one folder.

**Ralph's amendment (same sitting): keep AND improve the scanner.** "For
people who don't use Genius Scan, it's still a good enough option to have
it in our app. I don't want to remove it, but it would be great if we
could improve it." He's been frustrated with it "this whole time" and
wishes he understood what Genius Scan does differently.

**Failure mode pinned down (Ralph, same sitting):** NOT background
contrast — he already scans on contrasting surfaces. "It fails where the
music folds. The fold line doesn't come up as an edge that it detects."
I.e., orchestral parts live on large folded sheets; the page he wants ends
at a CREASE, not a paper edge, and Apple's detector hunts physical edges
against background — a fold in the middle of a bigger sheet is invisible
to it. This is a sheet-music-specific failure mode a generic scanner
will never prioritize. Kills improvement avenue (1) (capture coaching);
re-aims the fix at CROP ERGONOMICS, not detection: candidate = accept the
whole-sheet capture, then a fast post-scan crop step to the fold — PFN
already owns mature crop-box UI (house components; InlineCropper /
PassageRectDrawer family), so this is reusing the app's core competency
rather than fighting Apple's detector. Open question answered by Ralph:
"Genius Scan finds the edges and finds the fold by itself, no problem" —
their engine genuinely auto-detects creases. That IS the expensive part
of what he pays for; PFN will not match it. **Settled target (unratified
as a build, but the direction is clear): fast post-scan manual crop using
PFN's own crop UI, plus the receive-first strategy (F29 batch → F28
share-sheet) so Genius Scan remains the scanner of choice for those who
have it and PFN's scanner stays the good-enough built-in.**

**What the code says (checked live):** PFN's native scan = Apple's
VisionKit document scanner via `react-native-document-scanner-plugin`
(`app/document-upload.tsx`) — the same first-party scanner as the iOS
Notes app; we wrote none of the edge detection. Genius Scan runs its own
proprietary engine (~15 years of it; it IS their business, also sold as a
commercial SDK). Assessment for a future session, NOT verified on-device:
sheet music is close to edge-detection's worst case (white page, low
contrast, often on a white/light surface or stand), and Apple's detector
is known to struggle exactly there. Improvement avenues, cheap→expensive:
(1) capture coaching — "place the page on a dark, contrasting surface"
tip in the scan flow (Genius Scan's own support docs give the same
advice); (2) verify VisionKit's post-capture corner-adjust step is
actually reachable in our flow (it exists in the framework); (3) licensing
the Genius Scan SDK — real money, not sane at current revenue. Any real
work here starts with an on-device repro of the 70% failure (what
surface, what light), not code changes.

### F29 — Concert-folder filling wants BATCH PDF upload

- **What Ralph said (his words):** "when I'm filling up a folder for a
  concert, usually all the files have already been scanned. It would be
  nice to be able to select more than one PDF to upload to the library at
  a time."
- **Today:** the add-a-full-part flow ingests exactly one PDF per pass;
  filling a concert folder means repeating the whole pick-and-save loop per
  piece. Concert prep is inherently batchy (a season's program arrives as a
  stack of already-scanned parts).
- **Shape (unratified):** multi-select in the document picker → sequential
  ingest into the current folder, one library card each. Both platforms'
  pickers support multi-select; the work is in the ingest loop + progress
  UI. Pairs naturally with F28 (share several PDFs from Genius Scan at
  once) and with the folder-as-concert structure Ralph already uses
  (season → week folders = the deadline system, per the earlier
  folder-due-date insight).

### D56 — Maintenance-tempo exercises: the ladder has no "stay" setting

- **What Ralph did (his words):** "I am starting to get annoyed that there are
  certain exercises I always do at the same tempo. There's no goal to move
  them up, and I'm not really sure where to record that." Tried start 112 +
  climb-by 0 to make a flat ladder; the increment field refuses 0 (min is 1,
  `isValidIncrementStep` in TempoConfigFields requires 1–99); settled on 1
  "and see what happens." His word: "annoying."
- **What the app would have known:** nothing usable — the Tempo Ladder's
  entire data model assumes tempo PROGRESS is the goal. A fixed-tempo
  maintenance exercise (play it at 112, every day, forever) has no home: the
  workaround ladder will slowly creep the tempo upward (+1 per clean set),
  which is exactly what Ralph does NOT want, and the trail records a fake
  climb.
- **Why it matters:** third leg of the daily-routine picture. D53 = climbers
  that never graduate; D55 = material rotates in; D56 = some routine items
  should NOT climb at all — they are attendance + quality at a fixed tempo.
  The daily-exercises feature ([[project_pfn_daily_exercises_design]]) was
  framed as a long-horizon TEMPO TRACKER; this says it also needs a
  no-tempo-goal mode where the record is simply "done today at ♩=112."
- **Verdict: real gap, shape unratified.** Cheapest patch = allow increment 0
  (a flat ladder that never bumps). Real fix likely lives in the
  daily-exercises feature. Ralph's call which.

### F26 — Crash at save/finish on the D56 workaround session (= F12 again)

Sequence: Tempo Ladder on "Trills", custom pattern "One take" (1 rep), start
112, goal 120, increment 1 → clicked Clean → saved/finished → CRASH. Redid
the session; second save worked. Live-DB check (read-only): exactly ONE
practice_log row exists (id 2334, tempo_ladder, 12:39 Detroit — the redo);
the crashed first attempt did NOT persist. Matches the standing F12
intermittent save-crash race (~3-of-4 saves fine); crash-tax today = one
redone stint, no duplicate row, no corruption. F12 remains the open bug.

D56 follow-on confirmed in the data: the surviving row carries
increment 1 → per the +1-per-clean-set resume logic the NEXT session will
prefill Trills at 113, which Ralph explicitly does not want (maintenance
tempo is 112). The workaround creeps.

### D57 — The ladder session is a LOGGING RITUAL, not the practice itself

- **What Ralph did (his words):** "Each of these exercises is 3 pages long.
  I'm just marking the spot as the title of the exercise, setting the tempo
  and the tempo ladder, because that's the only way I can log that I've done
  it. I then click that I got it and log that as a practice session. I then
  come back to the PDF and start actually practicing the exercise."
- **What this means:** for daily-routine items the practice_log rows are
  ATTENDANCE RECEIPTS, not practice records. The 1-rep "clean" at ♩=112 is
  clicked as a bookkeeping act; the real practicing (3 pages, unknown
  duration, unknown tempos) happens afterward against the PDF, invisible to
  the trail. The durationMs stamped on these rows measures the ritual, not
  the work. Any coach rule reading rep counts / clean rates / duration on
  Daily Routine passages is reading ceremony, not practice.
- **Why it matters:** Ralph is hand-simulating a check-in feature ("I did
  Trills today at 112") with the heaviest tool in the app. The
  daily-exercises design's real job shrinks to: a one-tap "done today at
  ♩=N" per routine item — mark-the-spot + title + tempo memory + streak —
  with NO reps, sets, or climb mechanics unless the item wants them (D53
  climbers do; D56 maintainers don't).
- **Verdict: NO deterministic rule can survive on this trail** for routine
  items — the input data itself is ritual. Strengthens fix (c) from D53
  (build the feature) over patching coach rules around it.

### F27 — The document metronome always resets to 120 (wants: carry my tempo)

Back in the PDF viewer for the ACTUAL practicing, the metronome sat at 120.
Ralph wanted it "back where it had been during the ladder session" — 112,
the tempo he always does this exercise at, which he had JUST dialed in one
screen earlier. Root cause confirmed in code: the viewer's tool layer mounts
its own metronome hard-coded to 120 (`PracticeToolsLayer.tsx:215`,
`ownMetronome = useMetronome(120)`); it inherits nothing, every open starts
at 120. The ladder screen's metronome is a separate instance that dies with
that screen.

- **Candidate shapes (NOT ratified):** carry the user's last-set BPM across
  screens within a visit (one shared/session-remembered BPM), or per-passage
  /per-document tempo memory feeding the viewer metronome (rhymes with the
  2026-08-21 unified tempo prefill work, which fixed exactly this class of
  "the app forgot my tempo" for setup screens but never touched the
  free-floating viewer metronome).
- **Ralph's own framing:** the daily-exercises feature "will just be a
  workaround for all of this" — i.e. D56/D57/F27 might all dissolve if a
  routine item is a one-tap check-in with its own remembered tempo. Note the
  120-reset also bites OUTSIDE daily routine (any return to a document after
  a session), so the metronome-memory piece may deserve fixing regardless.

### D55 — Deliberate variety injection into the Daily Routine

- **What Ralph did (his words):** "I'm resisting doing the same things that
  I've been doing up until now, so I'm adding a resource to that folder" — a
  new finger exercise joins the Daily Routine folder today.
- **What the app would have known:** a new passage appears in the Daily
  Routine folder with zero history. To the coach it's indistinguishable from
  any cold-start passage; it has no way to know this is a ROUTINE SLOT being
  refreshed, not a new piece of repertoire.
- **Why it matters:** (1) It confirms the D53 finding from the other side —
  daily-routine passages have their own lifecycle (they get rotated IN as
  well as climbing forever). (2) It feeds the design-locked daily-exercises
  feature's open "grain + graduation" question: Ralph's real routine isn't a
  fixed set; anti-staleness rotation is part of correct fundamentals
  practice, and HE initiates it. (3) The coach's anti-rut instinct (D15,
  "introduce some variation") shows up here as swapping MATERIAL, not just
  swapping tools on the same material.
- **Verdict: watch the add flow.** How it feels to add a resource to the
  routine today IS the friction test for the daily-exercises design.
