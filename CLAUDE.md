...
- **`/import-supabase` route ported**: native data-import dev tool lives in the merged repo. `lib/supabase/client.ts` split (native is one-shot, `persistSession: false`; web is the existing config). `lib/supabase/import.ts` copied verbatim. `app/import-supabase.tsx` is the native UI; `app/import-supabase.web.tsx` is a placeholder explaining web is already Supabase-native. Route registered in `_layout.tsx`. Run via direct URL `/import-supabase` (no menu link). The "Replace what's on this iPad" toggle deletes all local SQLite rows before re-importing (it does NOT delete the app-sandbox `documents/` + `pieces/` directories — orphaned files linger on disk). ⚠️ **2026-08-12 data-loss incident**: this wipe deleted Ralph's iPad-only Frenzy project (the iPad is local-first with NO native→web sync, so iPad-created work exists nowhere else). Guard added same day: `runImport` now compares local ids against the fetched web rows and, if anything is local-only, blocks on a typed-DELETE confirmation modal (`confirmWipe` in `ImportOptions`) BEFORE any destructive step; cancelling aborts the import untouched. Root fix remains native→web sync (memory `project_ios_storage_rearchitecture`); a pre-wipe local backup/export is a noted follow-up.
...
Subscription ($39/yr / $4.99/mo) → **one-time $19.99 purchase, owned forever**. Verified first: **zero paid subscriptions exist** (77 active comp rows — **50 expire 2026-07-26**, 20 expire 2026-12-26, 7 lifetime), so no refunds/migration needed. Emails ALREADY sent 2026-06-26 (in Gmail): broad base promised free month → free plan for good → Pro at the old $39/yr; lifetime cohort promised free-for-life. **Ralph's call (2026-07-04): NO follow-up email** — the Jun 26 emails remain true; the 50 free-month users meet the $19.99 price on the paywall when comps end Jul 26; six-month cohort quietly keeps Pro to Dec 26 (pleasant surprise); lifetime stays lifetime. The one Stripe-connected row is Ralph's own BETA6 test sub.

**Mechanism (minimal-change):** a purchase writes the same `subscriptions` row shape the comps use — `tier 'pro', status 'active', current_period_end = 4102444800000` (lifetime sentinel, matches `LIFETIME_AFTER_MS`). `useSubscription` / `useEntitlement` logic unchanged; "did they ever buy" = unexpired pro row.

- `constants/billing.ts` — new pivot runbook (replaces the go-live one), `PRICE_LIFETIME_LABEL/_SUBLABEL`; retired `REWARD_MONTHS`/`TESTER_PROMO_CODE` (promo codes don't apply to one-time checkout); copy rewritten around "unlock forever".
- `lib/billing/checkout.{ts,web.ts}` — `startCheckout()` takes no plan; native stub message updated.
- `components/PaywallModal.tsx` — single "Unlock forever — $19.99" button + sublabel; beta-code fine print gone.
- `app/account.tsx` — paid tier reads "Practice Pro — unlocked, yours forever."; button "Unlock Practice Pro".
- `supabase/functions/create-checkout-session` — `mode: 'payment'`, env **`STRIPE_PRICE_LIFETIME`** (new secret, must be set before deploy); refuses to double-charge an already-active user (`?checkout=already` redirect); promo codes off.
...
- ~~founders-forever.sql~~ — **REVERSED by Ralph 2026-07-04 (`f74926f`, file deleted)**: the dated comps expire naturally (50 on Jul 26, 20 on Dec 26) → those users land on the free tier and can buy the $19.99 unlock like anyone else (lock-don't-lose already handles it; zero code). The 7 lifetime comps stay lifetime. Announcement email should say "free through December, then $19.99 once" — do NOT promise free-forever.

**Price = $19.99 (decided 2026-07-04 after competitor research: forScore $24.99 one-time, Modacity $129/yr — $9.99 was underpriced).** Stripe live one-time price created by Ralph: `price_1TpXSZC1HWgXmdPC0uihVt1z` (main account). **Steps 1–3 DONE 2026-07-04:** secret saved; both edge functions deployed (v11, ACTIVE, webhook verify_jwt=false; smoke: OPTIONS 200 / unsigned webhook 400). Web pushed 2026-07-04 (`91f3272`) — buttons and checkout now match. Follow-up `cffa8ba` (also live): signup card leads with free-month + email-is-only-for-saving reassurance BEFORE the email field (Ralph's test feedback); PRO_FEATURES gains the automatic-practice-log line. **Next:** Ralph's authed click-through (Account → Unlock Practice Pro → single $19.99 button; his comped account should bounce back via the double-charge guard); confirm BETA6 Stripe sub cancelled; Phase 2 = Apple IAP; Phase 3 = founders email + landing copy + founders-forever.sql.

## 🔨 2026-07-04 — Expiry-reminder emails (BUILT, not deployed — needed by Jul 23)

Ralph asked for an automated "your free month ends in 3 days" email to users + himself. Built via Resend (he has an account; domain verification + `RESEND_API_KEY` secret = his steps) + a daily pg_cron → edge function:
- `supabase/functions/expiry-reminder/index.ts` — daily 13:00 UTC; window = access ends in [now+2d, now+3d); covers dated comps AND 30-day trials (created_at+30d, skipping active comp/pro rows); lifetime/pro never match. Sends via Resend from ralph@playfastnotes.com, reply-to + BCC rdskiano@gmail.com; dedupe via insert-first into `expiry_reminders` (delete row on send failure so next run retries). Auth = `x-cron-secret` header (`CRON_SECRET` secret; generated value in the session where this was built — regenerate if lost). Email copy inside the function (Ralph should approve before deploy).
- `supabase/expiry-reminder-setup.sql` — dedupe table (RLS on, no policies) + `cron.schedule` (needs pg_cron + pg_net enabled; substitute the real CRON_SECRET). Run with approval at deploy time.
- `config.toml` — `[functions.expiry-reminder] verify_jwt = false`.
- Account screen: new comp-ending warning (`compEndingBody` in billing.ts) — dated comps ≤5 days out now get the same in-app heads-up trial users always had.
- ✅ DEPLOYED + LIVE 2026-07-04: domain verified in Resend (DNS at NAMECHEAP, not Vercel — nameservers registrar-servers.com; Mail Settings switched Email Forwarding→Custom MX, no forwarding was in use, so mail TO @playfastnotes.com bounces by design; replies covered by reply-to Gmail). Secrets set; function v1 ACTIVE; migration `expiry_reminders_setup` applied (pg_cron+pg_net enabled); cron job `expiry-reminder-daily` 13:00 UTC reads the secret from **Vault** (`expiry_cron_secret`) — rotate BOTH Vault row + function secret together. Test-mode send (POST {"test":true} + header) returned ok, Resend id issued — Ralph confirming inbox arrival. First real fire: Jul 23 (the 50 July comps). Local commit still needed; the account-screen comp warning rides the next web push (needed before ~Jul 21).

## ✅ 2026-08-15 — Starter seed retired + direct account creation (SHIPPED 2026-08-16 as part of the entry above)

Two coupled onboarding changes (Ralph's call: new users should land in an EMPTY library, and motivated visitors shouldn't be forced through the funnel):

1. **Bundled starter seed retired.** `seedIfEmpty()` (which planted Ralph's own April-2026 iPad export — 14 pieces, 26 sheet-music photos, 57 practice-log rows — into every fresh native install) is deleted; `assets/seeds/initial-seed.json` stays in the repo but is no longer imported, so it's out of the bundle (~5.7MB smaller). `lib/startup/migrate.ts` now calls **`cleanupStarterSeed()`** (`lib/db/seedCleanup.ts`): a one-time, OTA-deliverable cleanup that removes untouched starter rows + files from installs older binaries already seeded (App Store build 12's embedded JS still seeds on first launch; the OTA cleans on the next). Safety: never runs on a signed-in device or one with a recorded sync owner (Ralph's iPad is safe); per-piece it only deletes rows whose updated_at still matches the seed fingerprint (`lib/db/seedManifest.ts`, generated by `scripts/generate-seed-manifest.ts`); all deletes run under `withSyncApplying`. **Sim-verified 2026-08-15** (iPad Pro 11" dev client, JS from Metro): fresh install → 0 rows + marker + first-passage hero renders; planted seed with 1 edited piece + 1 practiced piece → exactly those 2 survive with their files, Ralph's 57 log rows purged, "For students" deleted, Ginastera kept, seeded settings removed, nothing added to sync_outbox; planted seed + `sync_state.owner_user_id` → everything untouched, marker set (permanent skip). The live-Supabase-session guard is untested (needs real credentials) — the owner guard covers any synced device on top of it. Note: `practice_log.id` is INTEGER — the manifest keeps those ids numeric (generator comment explains).
2. **Library onboarding redirect + "deciding" blank-hold are now web-only** (`app/(tabs)/library.tsx`). A fresh empty native install shows the first-passage hero instead of bouncing into the web-shaped funnel (which dead-ends on native at forced signup). Without the second gate the empty native library would have held a blank screen forever.
3. **Direct signup on `/sign-in`.** Login mode's footer now has (web) "Get started →" into the funnel PLUS "Skip the tour and create an account"; on native the direct path is the only signup (funnel is web-shaped). Direct mode = grouped instrument chips (required) + email + password; on success writes `onboarding:instrument` + `onboarding.seen`, does NOT seed Bumblebee (deliberate — empty library + hero), logs `signed_up` with `intent: 'direct'`, lands at `/library`. Deep-linkable as `/sign-in?signup=1`. Card now scrolls (ScrollView) so the taller signup form works on phones.

Verified: tsc clean, web + ios exports clean, web click-through on desktop + phone widths (login ↔ direct toggle, chip select, disabled submit until instrument+email+password). NOT verified: a real account creation end-to-end, and the native cleanup paths.

## ✅ 2026-08-16 — Onboarding funnel BOXED OFF; one setup screen everywhere (SHIPPED 2026-08-16: web push + OTA production)

Ralph's morning-after call (he rejected the 08-15 "second entrance" design): **no onboarding exercises at all**. New users on BOTH platforms get one setup screen — name + instrument + email + password — then an empty library. His reasoning: the funnel never fixed activation; tutorial videos are the real "aha" channel; friction is acceptable at 4-5 paying customers. Supersedes the 08-15 direct-signup entry below (that code evolved into this).

- **`constants/onboardingFunnel.ts`** — `ONBOARDING_FUNNEL_ENABLED = false`, the single switch that boxes off the funnel (nothing deleted). Gates: sign-in "Get started" button, `/onboarding` route (redirects to `/sign-in?signup=1` + renders null), library first-run redirect + `decidingFirstRun` blank-hold. The in-app "?" strategy demos are deliberately NOT gated.
- **Setup form** (`app/sign-in.tsx` signup mode): name (required) → instrument chips (required) → email → password. Uses new **`signUpWithProfile`** (`lib/supabase/auth.ts`): name+instrument ride as auth user metadata (`full_name`, `instrument`) so they survive the email-confirmation gap and cross-device confirms; handles confirmation ON (returns `confirm-email` → screen shows a check-your-email card with resend via `resendConfirmationEmail`) and OFF (session → writes settings → `/library`) so the code is safe to deploy before/after the dashboard flip. Existing-email attempts get try-password-then-friendly-error, including the confirmation-ON obfuscated path (`identities.length === 0`).
- **`lib/onboarding/applySignupProfile.ts`** — on first signed-in library load, copies metadata instrument → settings + marks `onboarding.seen`. Only acts when metadata.instrument exists (new-flow accounts), so existing accounts + demo account untouched. Called from library mount.
- **iPad sign-in wall** (`app/_layout.tsx`): the auth gate now applies to BOTH platforms (Ralph's decision, no skip option; name required also his call). Signed-in devices keep working offline (stored session counts). Un-signed-in installs with local data hit the wall; their data is preserved and syncs up after they sign in.
- **Verified**: tsc + both exports clean; web click-through (login footer = single "Create an account"; /onboarding redirects to setup form; full form renders, submit disabled until complete); iPad sim fresh install → sign-in wall → native setup form renders fully. **NOT verified**: real account creation end-to-end, and the check-email card live (needs the Supabase flip).
- **Ralph's dashboard steps at ship time (email confirmation)**: (1) Supabase → Authentication → Sign In / Providers → Email → enable "Confirm email". (2) Point Supabase auth emails at **Resend SMTP** (he has the account + verified playfastnotes.com domain) — Project Settings → Authentication → SMTP; Supabase's built-in sender is ~2-4 emails/hour and spam-prone. (3) Check the confirmation-email template + confirm Site URL/redirects point at playfastnotes.com. The app works with confirmation ON or OFF, so these can happen right before or right after the deploy.
...
4. **Document viewer pinch-zoom while marking** — DONE for web phones in DRAW mode (2026-07-08). New `components/PinchZoomPan.web.tsx` wraps the page + `PageBoxOverlay` + draw/draft surfaces in a CSS-transform zoom layer: **two fingers pinch/pan, one finger still draws**. The old worry (box math lives in un-zoomed slot coords) is handled by (a) `getBoundingClientRect` already reflecting the transform for the *final* source-pixel region, and (b) passing the live `zoom` scale into `PassageRectDrawer.web`/`PassageRectResizer.web` so their on-screen preview/drag-delta divide by it; `suspended`/`markPinching` makes the drawer/resizer yield the instant a 2nd finger lands (no half-drawn box). Native no-op sibling `PinchZoomPan.tsx`. **Still NOT done / possible follow-ups:** web *tablet* (scoped to `isPhone` to avoid spread-mode complications); *resize* mode (left alone so multi-page swipe still works — the touch-capturing wrapper would block it); *section* marking; and native draw-mode zoom (native already zooms in idle reading only). ⚠️ Multi-touch can't be verified on desktop — needs an on-phone test before the live web push.
