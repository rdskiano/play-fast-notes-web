...
⏳ **Still pending:**
- Bring in iPad-only components: `InlineMetronome` (used by interleaved / click-up / rhythmic on iPad), `CropView` (used by multi-page / crop), `RhythmNotation` (used by FloatingRhythmCard). The other three from the original "metronome internals" list — `NoteCardStrip`, `PreviewPlayButton`, `SubdivisionControls` — are orphans in the iPad archive itself (zero importers anywhere). Skip them. `Subdivision` type is re-exported from the new `lib/audio/useMetronome` on both platforms, so the iPad imports of `import type { Subdivision } from '@/lib/audio/useMetronome'` resolve cleanly here.
