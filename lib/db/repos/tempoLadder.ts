...
export async function deleteTempoLadderProgress(exerciseId: string): Promise<void> {
  const db = getDb();
  await db.runAsync(
    `DELETE FROM tempo_ladder_progress WHERE exercise_id = ?;`,
    exerciseId,
  );
}
...
export async function listSiblingLadderConfigs(
  ...
  const db = getDb();
  const rows = await db.getAllAsync<SiblingLadderConfig>(
    `SELECT sib.id AS piece_id, tp.mode AS mode, tp.start_tempo AS start_tempo,
