...
**🔨 In progress 2026-05-20 — floating practice tools + metronome redesign.** New `components/PracticeToolsLayer.tsx` mounts edge-docked tools on the passage and document screens. Each tool is a tab on a screen edge that pops out a draggable, pinch-resizable card that flies home to its tab on collapse (`components/ToolDock.tsx`). Left edge = Apple Pencil (placeholder) + Metronome; right edge = Timer (the real practice-timers pill, via `PracticeTimersPill bare`) + Tuner (placeholder). The Metronome was rebuilt Tempo-app style (`components/MetronomePanel.tsx`): volume bar, tappable per-beat accent/normal/mute dots, big BPM + tap-for-tempo, meter + subdivision pickers, circular play button. The engine gained a per-beat pattern (`setBeatPattern`); its default stays a single uniform accent so practice-flow callers (`FloatingMetronome`) are unchanged. Builds clean (`tsc` + `expo export -p ios`). **TODO:** device-test the redesign; recorder tool (bottom edge); make the metronome live on practice pages with the strategy driving it; remove the now-redundant standalone timer pill from the toolbars.
